#
/* Compile: cc -ansi -pedantic -o ppm2sprite ppm2sprite.c */

/**
 * This program converts a .PPM image file (Portable PixMap) to the 
 * ModelSim Mobsters .MIF (Memory Initialization File) sprite format.
 *
...
char *argv[];
{
	...
	fprintf(sprite, "-- Sprite: %s; size: %hux%hu pixels\n", argv[1], width, height);
	...
	for (i = 0; i < width * height; ++i, address++) {
		...
		zbgr = ((transparent & 0xF0) << 8) | ((b & 0xF0) >> 4) << 8 | ((g & 0xF0)) | (r & 0xF0) >> 4;

		fprintf(sprite, "%04hx : %04hx ;\n", address, zbgr);
	}	
